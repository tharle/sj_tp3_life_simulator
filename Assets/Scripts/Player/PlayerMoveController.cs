...
public class PlayerMoveController : MonoBehaviour
{
    ...
    private void Move()
    {
        ...
        //obtient la nouvelle direction ( (avant/arri�rre) + (gauche/droite) )
        Vector3 directionDep = GetCameraTransform().forward * vDeplacement + GetCameraTransform().right * hDeplacement;
        directionDep.y = 0; //pas de valeur en y , le cas o� la cam�ra regarde vers le bas ou vers le haut
        Vector3 velocity = Vector3.zero;
        if (directionDep != Vector3.zero) //change de direction s�il y a un changement
        {
            //Oriente le personnage vers la direction de d�placement et applique la v�locit� dans la m�me direction
    ...
    {
        // M�chant if � cause de "Reload Scene", �a donne un frame pour que la Cinemachine start � nouveau
